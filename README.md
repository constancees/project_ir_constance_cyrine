# Final Project - Autonomous Navigation with the JPL Rover - Constance Gagneraud, Cyrine NAOUAR

For our project, we decided to try to implement a PD (Proportional Derivative) controller and an FSM (Finite State Machine) to run our robot. The way this works is that the robot has 3 different states : FORWARD, REALIGN and TURN.
...
In the terminal, the robot will print the sensed values in front, to its left and to its right, as well as the current state it is in.
